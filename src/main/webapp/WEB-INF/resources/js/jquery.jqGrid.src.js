...
$.jgrid.extend({
				...
				$(obj.p.colModel).each( function(i) {
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && this.editable===true && nm !== 'rn') {
						...
						if ( trdata.length===0 ) {
							...
							//考虑edit form中字段顺序与列表中可能不一样 
							var findflag=false; 
							for(var x=rp+1;x<=maxRows;x++){
								var maxtrdata=$(tb).find("tr[rowpos="+x+"]");
								if(maxtrdata.length>0){ 
									findflag=true;
									break;
								}
							} 
							if(findflag) maxtrdata.before(trdata);
							else $(tb).append(trdata); 
							/////////  
							trdata[0].rp = rp;
						}
	...
	detailGridRow : function(rowid, divid,p){
		p = $.extend(true, { 
			beforeInitData : null,
			beforeShowForm : null
		}, $.jgrid.view, p || {});
		...
		return this.each(function(){
			...
			maxCols = 1,maxRows=0;  
			function createData(rowid,obj,tb,maxcols){
				var nm, hc,trdata, cnt=0, v_tr_cnt=0,tmp, dc, retpos=[], ind=false, i,
				tdtmpl = "<td class='CaptionTD form-view-label ui-widget-content'>&#160;</td><td class='DataTD form-view-data ui-helper-reset ui-widget-content'>&#160;</td>", tmpl="",
				tdtmpl2 = "<td class='CaptionTD form-view-label ui-widget-content'>&#160;</td><td class='DataTD form-view-data ui-widget-content'>&#160;</td>",
				fmtnum = ['integer','number','currency'],max1 =0, max2=0 ,maxw,setme, viewfld;
				for (i=1;i<=maxcols;i++) {
					tmpl += i === 1 ? tdtmpl : tdtmpl2;
					...
					if ( nm !== 'cb' && nm !== 'subgrid' && nm !== 'rn' && viewfld) {
						...
						if ( trdata.length===0 ) {
							...
							if(!hc){ 
								...
								v_tr_cnt++;
								//if(v_tr_cnt%2==0)
								if(rp%2==0)//要求隐藏字段设在一开始或最后
									trdata.addClass("Even_tr");
								else 
									trdata.addClass("Odd_tr"); 
							}
							$(trdata).append(tmpl); 
							
							//考虑detail form中字段顺序与列表中可能不一样 
							var findBiggerRowposTrFlag=false; 
							for(var x=rp+1;x<=maxRows;x++){
								var maxtrdata=$(tb).find("tr[rowpos="+x+"]");
								if(maxtrdata.length>0){ 
									findBiggerRowposTrFlag=true;
									break;
								}
							} 
							if(findBiggerRowposTrFlag) maxtrdata.before(trdata);
							else $(tb).append(trdata); 
							/////////  
							trdata[0].rp = rp;
						}
			...
			if ( $("#"+frmtb_id)[0] !== undefined ) { 
				fillData(rowid,$t); 
			} else {
				/*cellspacing='1' cellpadding='2' style='table-layout:fixed'*/
				var tbl =$("<table id='"+frmtb_id+"' class='DetailTable'  border='0' ><tbody></tbody></table>");
				
				if(onBeforeInit) {
					//onBeforeInit.call($t,$("#"+frmtb));
					//其他如editGridrow等中此方法中实际也取不到$("#"+frmtb)，也应修改成tbl才对
					onBeforeInit.call($t,tbl);
				}
				$($t.p.colModel).each( function() {
					var fmto = this.formoptions;
					maxCols = Math.max(maxCols, fmto ? fmto.colpos || 0 : 0 );
				...
				createData(rowid, $t, tbl, maxCols);  
				if($.isFunction(p.beforeShowForm)) {p.beforeShowForm.call($t,tbl);}	//$("#"+frmtb)
				$("#"+divid).append(tbl);
